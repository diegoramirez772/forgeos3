      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          {runs.map((run, ri) => {
            ...
            // Real history from tool events
            const history = (run.toolEvents ?? []).map(e => e.riskScore || 0)
            if (history.length === 0) history.push(0)
            if (history.length < 8) {
              // Pad with zeros for visual consistency
              while (history.length < 8) history.unshift(0)
            }
