...
import { Shield, CheckCircle, Save, AlertTriangle, XCircle, Clock, DollarSign, Lock, Zap, Eye, Ban } from 'lucide-react'
...
export function PolicyStudio() {
  const { policyPresets, toolPacks } = useAgentStore()
  ...
  useEffect(() => {
    if (policyPresets.length > 0 && !selected) {
      setSelected(policyPresets[0])
      setActivePreset(policyPresets[0].id)
    }
  }, [policyPresets, selected])
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-forge-surface border border-forge-border rounded-xl">
            ...
            <span className="text-xs text-forge-secondary font-medium">
              {(policyPresets.find(p => p.id === activePreset) || selected)?.name || 'Cargando...'} · {enabledCount} reglas activas
            </span>
          </div>
          {saved ? (
          ...
          ) : selected && selected.id !== activePreset ? (
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-4 gap-5">
          ...
          <div className="col-span-3">
            <motion.div key={selected?.id || 'none'} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
